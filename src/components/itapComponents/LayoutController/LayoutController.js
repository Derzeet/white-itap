            ...
            <div className="layoutControllerBody" style={{display: open?"flex":"none"}}>
                ...
                <div className="layoutOptions" style={{display: enabled?"flex":"none"}}>
                    <div >
                        ...
                        <TextField type={"Number"} value={levelSeparation} onChange={(event) => setLevelSeparation(parseInt(!isNaN(event.target.value) && !isNaN(parseFloat(event.target.value)) ? event.target.value : 0))}/>
                    </div>
